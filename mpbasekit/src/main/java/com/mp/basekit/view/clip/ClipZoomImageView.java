package com.mp.basekit.view.clip;
...
public class ClipZoomImageView extends AppCompatImageView implements ScaleGestureDetector.OnScaleGestureListener,
     ...
     * @return
     */
    public Bitmap clip(int width) {
        Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        Bitmap result = Bitmap.createBitmap(bitmap, mHorizontalPadding,
                getHVerticalPadding(), getWidth() - 2 * mHorizontalPadding,
                getWidth() - 2 * mHorizontalPadding);
        if (width != 0) {
            //将剪裁的图片压缩到size以下，如果没需求就注释该段代码
            result = ImageUtils.scaleBitmap(result, width);
        }
        return result;
    }

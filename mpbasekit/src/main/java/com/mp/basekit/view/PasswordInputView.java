...
public class PasswordInputView extends AppCompatEditText {
     ...
     * 边框风格
     */
    @IntDef({BorderStyle.BOX, BorderStyle.LINE, BorderStyle.FILL})
...
//        this(context, attrs, R.style.PasswordInputViewStyle);
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        ...
        boxWidth = t.getInt(R.styleable.PasswordInputView_pwv_box_width, DensityUtil.dip2px(24));
        ...
        switch (specWMode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                switch (specHMode) {
                    ...
                    case MeasureSpec.UNSPECIFIED:
                        height = (int) boxWidth;
                        width = height * maxLength + spacing * (maxLength - 1);
                        break;
            ...
            case MeasureSpec.EXACTLY:
                switch (specHMode) {
                    case MeasureSpec.AT_MOST:
                    case MeasureSpec.UNSPECIFIED:
                        height = (width - spacing * (maxLength - 1)) / maxLength;
                        break;
                    case MeasureSpec.EXACTLY:
                        height = Math.min((width - spacing * (maxLength - 1)) / maxLength, height);
                        width = height * maxLength + spacing * (maxLength - 1);
                        break;
                }
                break;
    ...
    private void checkRadius(int availableWidth, int availableHeight) {
        LogUtils.e("availableWidth:" + availableWidth + "---availableHeight:" + availableHeight);
        // 每个盒子的宽度 = （可用宽度 - 间隔宽度）/ 盒子个数
        boxWidth = (availableWidth - (maxLength - 1f) * spacing) / maxLength;
        LogUtils.e("boxWidth:" + boxWidth + "---maxLength:" + maxLength);

        float availableRadius = Math.min(availableHeight / 2f, boxWidth / 2f);
        boxWidth = availableRadius * 2;
        ...
        LogUtils.e("radius:" + radius + "---availableRadius:" + availableRadius);
    }
...
////        super.onDraw(canvas); // 去掉EditText默认的绘制
        ...
        if (drawHeight > boxWidth) {
            top = top + (drawHeight - boxWidth) / 2;
        }
        float start = getPaddingStart();
        int drawWith = getWidth() - getPaddingStart() - getPaddingEnd();
        if (drawWith > boxWidth * maxLength + spacing * (maxLength - 1)) {
            start = start + (drawWith - (boxWidth * maxLength + spacing * (maxLength - 1))) / 2;
        }
        float left;

        for (int i = 0; i < maxLength; i++) {
            left = start + (boxWidth + spacing) * i;
            rectF.set(left, top, left + boxWidth, top + boxWidth);
            drawBorder(canvas, i);
    ...
    private void drawBorder(Canvas canvas, int index) {
        ...
        switch (borderStyle) {
            case BorderStyle.FILL:// 填充
            case BorderStyle.BOX:// 边框模式
                if (borderStyle == BorderStyle.FILL) {

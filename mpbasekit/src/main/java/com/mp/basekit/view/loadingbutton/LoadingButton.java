package com.mp.basekit.view.loadingbutton;
...
public class LoadingButton extends DrawableTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mLoadingText = array.getString(R.styleable.LoadingButton_loadingText);
        ...
        mButtonText = getText().toString().trim();
    }
    ...
    private void restoreStatus() {
        ...
        if (enableShrink) {
            ...
            LogUtils.e("getLayoutParams().width：" + getLayoutParams().width + "    height：" + getLayoutParams().height);
        }
        requestLayout();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (curStatus == STATUS.IDE && enableShrink) {
            mRootViewSizeSaved[0] = getWidth();
            mRootViewSizeSaved[1] = getHeight();
            LogUtils.e("onLayout().width：" + getWidth() + "    height：" + getHeight());
        }
    }

...
public final class SHA256 {
    ...
    public static byte[] blake2b256(byte[] personal, int size, byte[] data) {
        byte[] result = new byte[32];
        try {
            Blake2bDigest digest = new Blake2bDigest(null, size, null, personal);
            digest.update(data, 0, data == null ? 0 : data.length);

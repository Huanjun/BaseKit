...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = MPUtils.getApp()
                    .getPackageManager()
                    .getPackageInfo(MPUtils.getApp().getPackageName(), 0);
            if (pi != null) {
        ...
        UNCAUGHT_EXCEPTION_HANDLER = new UncaughtExceptionHandler() {
            ...
            public void uncaughtException(final Thread t, final Throwable e) {
                ...
                Log.e("CrashUtils", "create " + fullPath + "!");
                if (createOrExistsFile(fullPath)) {
                    input2File(crashInfo, fullPath);
                    Log.e("CrashUtils", "create " + fullPath + "suc!");
                } else {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && MPUtils.getApp().getExternalCacheDir() != null)
            defaultDir = MPUtils.getApp().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = MPUtils.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }

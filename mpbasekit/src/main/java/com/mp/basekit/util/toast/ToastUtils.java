...
import com.mp.basekit.util.toast.anim.PathAnimView;
import com.mp.basekit.util.toast.style.ToastAntpoolStyle;
import com.mp.basekit.util.toast.style.ToastBlackStyle;
...
public final class ToastUtils {
    ...
    public static void init(Application application) {
        init(application, new ToastAntpoolStyle());
    }
    ...
    public static synchronized void show(CharSequence text) {
        ...
        sStrategy.show(text, IToastStrategy.TOAST_NORMAL);
    }
     ...
     * @param text 需要显示的文本
     */
    public static synchronized void showSuc(CharSequence text) {
        checkToastState();
     ...
     */
    private static View createTextView(Context context, IToastStyle style) {
        ...
        PathAnimView toastImage = new PathAnimView(context);
        toastImage.setTag("WarningAnim");
        //set imageLayout
        toastImage.setDuration(1000);
        LinearLayout.LayoutParams imageParams = new LinearLayout.LayoutParams(DensityUtil.dip2px(context, 16f)
                , DensityUtil.dip2px(context, 16f));
        imageParams.setMargins(0, DensityUtil.dip2px(context, 1f), DensityUtil.dip2px(context, 5f), 0);
        //config image
        toastImage.setColor(style.getTextColor());
        ...
        // 设置圆角大小
        drawable.setCornerRadius(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, style.getCornerRadius(), context.getResources().getDisplayMetrics()));

        TextView textView = new TextView(context);
        textView.setId(android.R.id.message);
        textView.setTextColor(style.getTextColor());
        textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, style.getTextSize(), context.getResources().getDisplayMetrics()));

        toastLayout.setPadding((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, style.getPaddingLeft(), context.getResources().getDisplayMetrics()),
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, style.getPaddingTop(), context.getResources().getDisplayMetrics()),
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, style.getPaddingRight(), context.getResources().getDisplayMetrics()),
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, style.getPaddingBottom(), context.getResources().getDisplayMetrics()));
        ...
        toastLayout.addView(textView, new LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT
